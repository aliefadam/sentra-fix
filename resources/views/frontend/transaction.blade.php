                                ...
                                @foreach ($transaction->transactionDetails as $detail)
                                    @if ($detail->shipping_status == 'waiting')
                                        <button data-modal-target="crud-modal" data-modal-toggle="crud-modal" type="button"
                                            ...
                                            class="btn-batalkan-pesanan w-fit bg-white border border-red-700  text-red-700 hover:bg-red-50 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-red-600 dark:hover:bg-red-700 focus:outline-none dark:focus:ring-red-800">
                                            Batalkan Pesanan
    ...
    <div id="crud-modal" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative p-4 w-full max-w-md max-h-full">
            ...
            <div class="relative bg-white rounded-lg shadow-sm dark:bg-gray-700">
                ...
                <!-- Modal body -->
                <form class="p-4 md:p-5" id="form-pembatalan">
                    @csrf
                    <div class="mb-5">
                        <label for="reason" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Alasan
                            Pembatalan</label>
                        <select id="reason" name="reason"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-pink-500 focus:border-pink-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-pink-500 dark:focus:border-pink-500">
                            <option selected="">-- Pilih --</option>
                            <option value="Tidak ada stock">Tidak ada stock</option>
                    ...
                    <button type="submit"
                        class="text-white inline-flex items-center bg-pink-700 hover:bg-pink-800 focus:ring-4 focus:outline-none focus:ring-pink-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-pink-600 dark:hover:bg-pink-700 dark:focus:ring-pink-800">
                        Konfirmasi
                    </button>
                </form>
            </div>
...
@section('script')
    <script>
        ...
        $(".btn-batalkan-pesanan").click(setRouteCancelTransaction);
        $("#form-pembatalan").submit(cancelTransaction);
        ...
        function setRouteCancelTransaction() {
            const transactionId = $(this).data('transaction-id');
            const detailTransactionId = $(this).data('transaction-detail-id');
            $("#form-pembatalan").attr("data-transaction-id", transactionId);
            $("#form-pembatalan").attr("data-transaction-detail-id", detailTransactionId);
        }
        ...
        function cancelTransaction(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: "PUT",
                        url: `/transaction/${transactionId}/${detailTransactionId}/cancel`,
                        data: data,
                        beforeSend: function() {
                            Swal.fire({
                                title: "Loading",
